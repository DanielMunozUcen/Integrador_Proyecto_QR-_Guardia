...
$result = $conexion->query("SELECT correo FROM usuario WHERE correo = '$email'");
if ($result->num_rows > 0) {
    ...
    if ($enviado) {
        ...
        header("Location: ../login.php?correo_no_encontrado=false");
        exit();
    }
} else {
    // Redirigir a login.php con un parámetro para mostrar la alerta de correo no encontrado
    header("Location: ../login.php?correo_no_encontrado=true");
    exit();
}
?>